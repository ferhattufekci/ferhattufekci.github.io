    ...
    function init(options) {
        ...
        $('.pt-trigger').on("click", function (e) {
            ...
            var pageTrigger = $(this),
                targetHref = pageTrigger.attr('href'),
                // Some .pt-trigger elements live outside the main menu
                // (e.g. the header logo/photo). Resolve the matching
                // .site-main-menu link for the target page so that one
                // gets the 'active' highlight instead of whatever
                // non-menu element happened to be clicked.
                menuLinkForTarget = $(menu + ' a[href="' + targetHref + '"]').first();

            activeMenuItem( menuLinkForTarget.length ? menuLinkForTarget : pageTrigger );
    ...
    function ajaxLoader() {
        ...
        var href = $('.ajax-page-load').each(function(){
            ...
            if(location.hash == location.hash.split('/')[0] + '/' + href.substr(0,href.length-5)){
                ...
                showContent();
                ajaxLoadedContent.load(toLoad + ' .zoom-container');
                return false;
            }
        });
