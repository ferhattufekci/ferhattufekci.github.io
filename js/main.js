...
(function ($) {
  ...
  // Keep the toggle button (icon/aria) and body scroll lock in sync
  // with whether the mobile drawer is open. Single source of truth so
  // every entry point (toggle click, overlay click, Escape key, nav
  // click, window resize) ends up in the same, consistent state.
  ...
  $(window)
    .on("load", function () {
      //Load
      // Preloader fade-out is handled by js/preloader.js, which also
      // drives the percentage shown while the page is loading.
  ...
  $(document).on("ready", function () {
    ...
    // Close the menu on overlay click, Escape key, or any in-page
    // navigation (main menu links, header logo/title, or any future
    // .pt-trigger element)
    $(document).on("click", ".pt-trigger, .mobile-menu-overlay", function () {
      mobileMenuHide();
    });
    ...
    $(document).on("keydown", function (e) {
      ...
      if (isEscape && !$("#site_header").hasClass("mobile-menu-hide")) {
        mobileMenuHide();
    ...
    // NOTE: The original Kerge template shipped a "Google Maps" init call
    // here ($("#map").googleMap(...)), but the page has no #map element
    // and the googleMap jQuery plugin was never included in this project.
    // That combination threw "TypeError: $(...).googleMap is not a
    // function" on every page load. Removed as dead code — this site
    // doesn't use an embedded map.
  });
})(jQuery);
